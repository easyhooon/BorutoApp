...
// remoteMediator test class 를 구현할때 application context가 필요하기 때문에 androidtest패키지내에서 test 진행
// remoteMediator unit test 의 목적: load 함수가 올바른 mediator result 를  반환하는지 확인
class FakeBorutoApi2: BorutoApi {
    ...
    private fun calculate(page: Int): Map<String, Int?> {
        // remoteMediator 를 테스트 하기 위한 트릭
        // 다음 페이지에 대해 null을 수신하도록 (remoteMediator receive null for next page)
        if (page1.isEmpty()) {
